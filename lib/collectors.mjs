...
import {parseBenchmarks} from './benchmarks.mjs';
...
export function parseSource(source,body,now=new Date().toISOString()) {
  const candidate=(id,title,summary,eventAt,extra={})=>({id:`${source.id}:${id}`,sourceId:source.id,title:compact(title).slice(0,240),summary:compact(summary).slice(0,850),observedAt:eventAt??null,retrievedAt:now,evidenceClass:'INFERRED',status:'candidate',category:source.category,sourceUrl:source.endpoint||source.url,...extra});
  const measurements=parseBenchmarks(source,body);
  if(measurements)return measurements.map(m=>candidate(m.id,`${m.label} · ${m.referencePeriod}`,`${m.geography}: ${m.value} ${m.unit}. ${m.comparison}; ${m.seasonalAdjustment}. Reference period ${m.referencePeriod}; company applicability requires review.`,null,{measurement:m,dateType:'Reference period is in the measurement; publication date not inferred',scope:'Public benchmark, not a Watershed price, volume, contract or operational fact.'}));
  if(source.adapter==='osm') {
    const data=JSON.parse(body);if(!Array.isArray(data.elements)||!Number.isFinite(Date.parse(data.osm3s?.timestamp_osm_base)))throw Error('OSM response or database timestamp missing');
    if(data.remark)throw Error('OSM returned a partial or remarked response');
    return [candidate('geographic-snapshot',`${source.name}: mapped corridor update`,`${data.elements.length} mapped elements in the fixed public regional query. Review mapping changes before changing the saved geography. This is not a traffic or facility incident.`,null,{mapDatabaseAt:data.osm3s.timestamp_osm_base,dateType:'OSM database timestamp is not an event date',scope:'OpenStreetMap community observations. Nearby roads or rail do not establish company use, legal truck access, current condition or route independence.'})];
  }
...
export async function collectDue(sources,states=[],options={}) {
  ...
  const due=sources.filter(s=>!['manual','key'].includes(s.adapter)&&(options.force===true||!byId.has(s.id)||(now-Date.parse(byId.get(s.id).fetchedAt))/3600000>=s.cadenceHours)).sort((a,b)=>a.priority-b.priority);
